...
app.post('/:id/generate-image', async (c) => {
  ...
  try {
    ...
    const genId = await generateImage({ characterId: id, dramaId: char.dramaId, prompt, model: body.model, size: CHARACTER_IMAGE_SIZE, resolution: body.resolution, configId: body.config_id ?? ep.imageConfigId ?? undefined })
...
app.post('/batch-generate-images', async (c) => {
  ...
  for (const cid of ids) {
    ...
    const prompt = finalPrompt || characterImagePrompt(char, stylePrompt)
    try {
      const genId = await generateImage({ characterId: cid, dramaId: char.dramaId, prompt, model: body.model, size: CHARACTER_IMAGE_SIZE, resolution: body.resolution, configId: body.config_id ?? ep.imageConfigId ?? undefined })
      results.push(genId)
    } catch {}
  }
