...
- **自动拆分（默认）**：用户没有明确提供自定义参数时，继续按剧本节拍、对白时长和视频模型限制自动决定分镜数量与时长，使用 `save_storyboards` 的整集替换流程。
- **自定义拆分**：只有用户明确选择自定义模式时，才参考消息中的目标总时长、允许范围和目标分镜数；目标数量是软参考，不得为了凑数制造无效镜头。
- **当前分镜重新拆分**：如果用户只要求重新拆分选中的分镜，只重新组织该分镜内部的 `【镜头N】`、`duration`、`atmosphere`、绑定和 `video_prompt`，调用单分镜更新工具；禁止清空整集、禁止调用 `save_storyboards` 的 `replace_existing`。
...
- 用户消息会给出目标视频模型；分镜拆解时先按 `prompt-generator/video-prompt` 中的 Seedance 2.5、Seedance 2.0 或 MiniMax H3 格式支线同步生成 `video_prompt`，只有未命中这三类时才使用通用 3 秒格式；不要混用模型方言。
- 模型格式只改变 `video_prompt` 的写法，不改变 `description` 的子镜头来源、角色/场景/道具绑定和单段不跨场景约束。
