...
test('brand rename changes visible names but preserves app identity and data paths', () => {
  ...
  assert.match(layout, /https:\/\/github\.com\/kongxg888\/huobao-drama/)
  assert.match(feed, /pkg\.productName/)
  assert.match(feed, /kongxg888\/huobao-drama/)
  assert.match(updater, /readdirSync\(tmpExtract/)
  assert.match(updater, /https:\/\/github\.com\/kongxg888\/huobao-drama\/releases\/latest\/download\/latest\.json/)
