...
test('brand rename changes visible names but preserves app identity and data paths', () => {
  ...
  assert.match(layout, /https:\/\/github\.com\/kongxg888\/jixiang-drama/)
  assert.match(feed, /pkg\.productName/)
  assert.match(feed, /kongxg888\/jixiang-drama/)
  assert.match(updater, /readdirSync\(tmpExtract/)
  assert.match(updater, /https:\/\/github\.com\/kongxg888\/jixiang-drama\/releases\/latest\/download\/latest\.json/)
  assert.doesNotMatch(updater, /chatfire-AI|installer\.chatfire\.site/)
